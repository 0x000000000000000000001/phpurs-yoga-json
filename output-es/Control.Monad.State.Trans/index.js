...
const StateT = x => x;
const withStateT = f => v => Control$dSemigroupoid.composeImpl(v)(f);
const runStateT = v => v;
...
const mapStateT = f => v => Control$dSemigroupoid.composeImpl(f)(v);
...
const monadReaderStateT = dictMonadReader => {
  ...
  return {local: Control$dSemigroupoid.composeImpl(mapStateT)(dictMonadReader.local), MonadAsk0: () => monadAskStateT1};
};
...
const monadEffectState = dictMonadEffect => {
  ...
  return {
    liftEffect: Control$dSemigroupoid.composeImpl((() => {
      ...
      return m => s => Bind1.bind(m)(x => Monad0.Applicative0().pure(Data$dTuple.$Tuple(x, s)));
...
const monadStateStateT = dictMonad => {
  ...
  return {state: f => Control$dSemigroupoid.composeImpl(pure)(f), Monad0: () => monadStateT1};
};
const monadTellStateT = dictMonadTell => {
  ...
  return {
    tell: Control$dSemigroupoid.composeImpl((() => {
      ...
      return m => s => Bind1.bind(m)(x => Monad1.Applicative0().pure(Data$dTuple.$Tuple(x, s)));
    })())(dictMonadTell.tell),
