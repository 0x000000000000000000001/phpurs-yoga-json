...
const monadReaderContT = dictMonadReader => {
  ...
  return {
    local: f => v => k => Bind1.bind(ask)(r => dictMonadReader.local(f)(v(Control$dSemigroupoid.composeImpl(dictMonadReader.local(v$1 => r))(k)))),
...
const monadStateContT = dictMonadState => {
  ...
  const monadContT1 = monadContT(Monad0);
  return {
    state: Control$dSemigroupoid.composeImpl((() => {
      const Bind1 = Monad0.Bind1();
      return m => k => Bind1.bind(m)(k);
    })())(dictMonadState.state),
    Monad0: () => monadContT1
  };
};
