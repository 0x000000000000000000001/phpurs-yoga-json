...
const mapMaybe = dictOrd => Control$dSemigroupoid.composeImpl(mapMaybeWithKey(dictOrd))(Data$dFunction.const);
...
const toUnfoldableUnordered = dictUnfoldable => Control$dSemigroupoid.composeImpl(dictUnfoldable.unfoldr(stepUnfoldrUnordered))(toMapIter);
...
const toUnfoldable = dictUnfoldable => Control$dSemigroupoid.composeImpl(dictUnfoldable.unfoldr(stepUnfoldr))(toMapIter);
const showMap = dictShow => dictShow1 => {
  const $0 = Data$dShow.showArrayImpl(v => "(Tuple " + dictShow.show(v._1) + " " + dictShow1.show(v._2) + ")");
  return {show: as => "(fromFoldable " + $0(Control$dSemigroupoid.composeImpl(Data$dUnfoldable.unfoldableArray.unfoldr(stepUnfoldr))(toMapIter)(as)) + ")"};
};
...
const filter = dictOrd => Control$dSemigroupoid.composeImpl(filterWithKey(dictOrd))(Data$dFunction.const);
...
const catMaybes = dictOrd => Control$dSemigroupoid.composeImpl(mapMaybeWithKey(dictOrd))(Data$dFunction.const)(identity1);
...
const bindMap = dictOrd => {
  ...
  return {
    bind: m => f => mapMaybeWithKey(dictOrd)(k => Control$dSemigroupoid.composeImpl((() => {
      const go = go$a0$copy => {
        let go$a0 = go$a0$copy, go$c = true, go$r;
        while (go$c) {
          const v = go$a0;
