...
const semigroupApp = dictApply => dictSemigroup => {
  ...
  return {append: v => v1 => dictApply.apply(dictApply.Functor0().map(append)(v))(v1)};
};
...
const monoidApp = dictApplicative => {
  ...
  return dictMonoid => {
    ...
    const semigroupApp2 = {append: v => v1 => $0.apply($0.Functor0().map(append)(v))(v1)};
...
const eqApp = dictEq1 => dictEq => ({eq: x => y => dictEq1.eq1(dictEq)(x)(y)});
const ordApp = dictOrd1 => {
  const $0 = dictOrd1.Eq10();
  return dictOrd => {
    const $1 = dictOrd.Eq0();
    const eqApp2 = {eq: x => y => $0.eq1($1)(x)(y)};
    return {compare: x => y => dictOrd1.compare1(dictOrd)(x)(y), Eq0: () => eqApp2};
  };
};
const eq1App = dictEq1 => ({eq1: dictEq => x => y => dictEq1.eq1(dictEq)(x)(y)});
const ord1App = dictOrd1 => {
  const $0 = dictOrd1.Eq10();
  const eq1App1 = {eq1: dictEq => x => y => $0.eq1(dictEq)(x)(y)};
  return {compare1: dictOrd => x => y => dictOrd1.compare1(dictOrd)(x)(y), Eq10: () => eq1App1};
};
