...
const unsafeReadProp = dictMonad => {
  ...
  return k => value => unsafeReadPropImpl(
    Control$dSemigroupoid.composeImpl(Control$dMonad$dExcept$dTrans.monadThrowExceptT(dictMonad).throwError)(Data$dList$dNonEmpty.singleton)(Foreign.$ForeignError(
      "TypeMismatch",
      "object",
      Foreign.typeOf(value)
    )),
    pure,
    k,
    value
...
const indexableExceptT = dictMonad => {
  const bindExceptT = Control$dMonad$dExcept$dTrans.bindExceptT(dictMonad);
  return {ix: dictIndex => f => i => bindExceptT.bind(f)(a => dictIndex.index(a)(i))};
};
