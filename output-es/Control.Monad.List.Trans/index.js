...
const wrapEffect = dictFunctor => v => dictFunctor.map(Control$dSemigroupoid.composeImpl(Skip)(Control$dSemigroupoid.composeImpl(Data$dLazy.defer)(Data$dFunction.const)))(v);
...
const zipWith$p = dictMonad => {
  ...
  const Bind1 = dictMonad.Bind1();
  return f => fa => fb => Functor0.map(Control$dSemigroupoid.composeImpl(Skip)(Control$dSemigroupoid.composeImpl(Data$dLazy.defer)(Data$dFunction.const)))(Bind1.bind(uncons(dictMonad)(fa))(ua => Bind1.bind(uncons(dictMonad)(fb))(ub => {
...
const monadEffectListT = dictMonadEffect => {
  ...
  return {liftEffect: Control$dSemigroupoid.composeImpl(fromEffect(Monad0.Applicative0()))(dictMonadEffect.liftEffect), Monad0: () => monadListT1};
};
const monadSTListT = dictMonadST => {
  const Monad0 = dictMonadST.Monad0();
  const monadListT1 = {Applicative0: () => ({pure: singleton(Monad0.Applicative0()), Apply0: () => applyListT(Monad0)}), Bind1: () => bindListT(Monad0)};
  return {liftST: Control$dSemigroupoid.composeImpl(fromEffect(Monad0.Applicative0()))(dictMonadST.liftST), Monad0: () => monadListT1};
};
