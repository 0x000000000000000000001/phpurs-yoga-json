...
const toAff = p => Effect$dAff._makeAff(
  Effect$dAff.isLeft,
...
const rm = /* #__PURE__ */ toAff1(Node$dFS$dAsync.rm);
...
const lstat = /* #__PURE__ */ toAff1(Node$dFS$dAsync.lstat);
...
const access$p = path => mode => Effect$dAff._makeAff(
  Effect$dAff.isLeft,
  Effect$dAff.unsafeFromLeft,
  Effect$dAff.unsafeFromRight,
  Data$dEither.Left,
  Data$dEither.Right,
  k => {
    const $0 = Control$dSemigroupoid.composeImpl(k)(Data$dEither.Right);
    return () => {
      Node$dFS$dAsync.accessImpl(path, mode, err => $0(Data$dNullable.nullable(err, Data$dMaybe.Nothing, Data$dMaybe.Just))());
      return Effect$dAff.nonCanceler;
    };
  }
);
const access = path => Effect$dAff._makeAff(
  Effect$dAff.isLeft,
  Effect$dAff.unsafeFromLeft,
  Effect$dAff.unsafeFromRight,
  Data$dEither.Left,
  ...
  k => {
    const $0 = Control$dSemigroupoid.composeImpl(k)(Data$dEither.Right);
    return () => {
      Node$dFS$dAsync.accessImpl(path, Node$dFS$dConstants.f_OK, err => $0(Data$dNullable.nullable(err, Data$dMaybe.Nothing, Data$dMaybe.Just))());
      return Effect$dAff.nonCanceler;
    };
  }
);
