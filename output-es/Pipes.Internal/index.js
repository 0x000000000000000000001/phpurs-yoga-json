...
const observe = dictMonad => {
  ...
  return p0 => {
    const go = p => {
      if (p.tag === "Request") { return Applicative0.pure($$$Proxy("Request", p._1, Control$dSemigroupoid.composeImpl(observe(dictMonad))(p._2))); }
      if (p.tag === "Respond") { return Applicative0.pure($$$Proxy("Respond", p._1, Control$dSemigroupoid.composeImpl(observe(dictMonad))(p._2))); }
      if (p.tag === "M") { return Bind1.bind(p._1)(go); }
      if (p.tag === "Pure") { return Applicative0.pure($$$Proxy("Pure", p._1)); }
      $runtime.fail();
    };
  ...
  hoist: dictMonad => {
    ...
    return nat => p0 => {
      const go = p => {
        ...
        if (p.tag === "M") { return $$$Proxy("M", nat(Bind1.bind(p._1)(p$p => Applicative0.pure(go(p$p))))); }
...
const monadTransProxy = {
  lift: dictMonad => {
    const Functor0 = dictMonad.Bind1().Apply0().Functor0();
    return m => $$$Proxy("M", Functor0.map(Pure)(m));
  }
};
  ...
  return {
    map: f => p0 => {
      const go = p => {
        if (p.tag === "Request") { return $$$Proxy("Request", p._1, x => go(p._2(x))); }
        if (p.tag === "Respond") { return $$$Proxy("Respond", p._1, x => go(p._2(x))); }
        if (p.tag === "M") { return $$$Proxy("M", Bind1.bind(p._1)(v => Applicative0.pure(go(v)))); }
...
const proxyAlt = dictMonadPlus => {
  ...
  const Alt0 = Alternative1.Plus1().Alt0();
  const Monad0 = dictMonadPlus.Monad0();
  const Bind1 = Monad0.Bind1();
  const Applicative0 = Alternative1.Applicative0();
  ...
  return {
    alt: v => v1 => {
      ...
      if (v.tag === "M") { return $$$Proxy("M", Alt0.alt(Bind1.bind(v._1)(p$p => Applicative0.pure(proxyAlt(dictMonadPlus).alt(p$p)(v1))))(Applicative0.pure(v1))); }
      $runtime.fail();
    },
...
const semigroupProxy = dictMonad => {
  const Bind1 = dictMonad.Bind1();
  const Applicative0 = dictMonad.Applicative0();
  ...
  return dictSemigroup => (
    {
      append: p1 => p2 => {
        const go = p => {
          ...
          if (p.tag === "Pure") {
            const $0 = p._1;
            return functorProxy1.map(v => dictSemigroup.append($0)(v))(p2);
          }
          $runtime.fail();
        };
        return go(p1);
...
const monoidProxy = dictMonad => {
  const semigroupProxy1 = semigroupProxy(dictMonad);
  return dictMonoid => {
    const semigroupProxy2 = semigroupProxy1(dictMonoid.Semigroup0());
  ...
  return {
    apply: pf0 => px => {
      const go = pf => {
        ...
        if (pf.tag === "M") { return $$$Proxy("M", Bind1.bind(pf._1)(v => Applicative0.pure(go(v)))); }
  ...
  embed: dictMonad => {
    ...
    return f => {
      const go = p => {
        if (p.tag === "Request") { return $$$Proxy("Request", p._1, x => go(p._2(x))); }
        if (p.tag === "Respond") { return $$$Proxy("Respond", p._1, x => go(p._2(x))); }
        if (p.tag === "M") { return bindProxy1.bind(f(p._1))(go); }
        if (p.tag === "Pure") { return $$$Proxy("Pure", p._1); }
        $runtime.fail();
      };
      return go;
  ...
  return {
    tailRecM: f => a0 => {
      const go = v => {
        ...
        if (v.tag === "M") { return $$$Proxy("M", Bind1.bind(v._1)(v1 => Applicative0.pure(go(v1)))); }
...
const proxyMonadAsk = dictMonadAsk => {
  const monadProxy1 = monadProxy(dictMonadAsk.Monad0());
  return {ask: monadTransProxy.lift(dictMonadAsk.Monad0())(dictMonadAsk.ask), Monad0: () => monadProxy1};
};
  ...
  return {
    local: f => {
      const go = p => {
        ...
        if (p.tag === "Pure") { return $$$Proxy("Pure", p._1); }
        if (p.tag === "M") { return $$$Proxy("M", Bind1.bind(dictMonadReader.local(f)(p._1))(r => Applicative0.pure(go(r)))); }
        $runtime.fail();
      };
      return go;
    },
...
const proxyMonadState = dictMonadState => {
  ...
  return {state: Control$dSemigroupoid.composeImpl(monadTransProxy.lift(Monad0))(dictMonadState.state), Monad0: () => monadProxy1};
};
const proxyMonadTell = dictMonoid => dictMonadTell => {
  ...
  return {tell: Control$dSemigroupoid.composeImpl(monadTransProxy.lift(Monad1))(dictMonadTell.tell), Semigroup0: () => Semigroup0, Monad1: () => monadProxy1};
};
const proxyMonadWriter = dictMonoid => dictMonadWriter => {
  ...
  const proxyMonadTell1 = proxyMonadTell(dictMonadWriter.Monoid0())(MonadTell1);
  return {
    listen: p0 => {
      const go = p => w => {
        ...
        if (p.tag === "M") { return $$$Proxy("M", Bind1.bind(dictMonadWriter.listen(p._1))(v => Applicative0.pure(go(v._1)(Semigroup0.append(w)(v._2))))); }
        $runtime.fail();
      };
      return go(p0)(Monoid0.mempty);
    },
    pass: p0 => {
      const go = p => w => {
        if (p.tag === "Request") { return $$$Proxy("Request", p._1, a => go(p._2(a))(w)); }
        if (p.tag === "Respond") { return $$$Proxy("Respond", p._1, b$p => go(p._2(b$p))(w)); }
        if (p.tag === "Pure") { return $$$Proxy("M", dictMonadWriter.pass(Applicative0.pure(Data$dTuple.$Tuple($$$Proxy("Pure", p._1._1), v => p._1._2(w))))); }
        if (p.tag === "M") { return $$$Proxy("M", Bind1.bind(dictMonadWriter.listen(p._1))(v => Applicative0.pure(go(v._1)(Semigroup0.append(w)(v._2))))); }
        $runtime.fail();
      };
      return go(p0)(Monoid0.mempty);
...
const proxyMonadThrow = dictMonadThrow => {
  ...
  const monadProxy1 = monadProxy(Monad0);
  return {throwError: Control$dSemigroupoid.composeImpl(monadTransProxy.lift(Monad0))(dictMonadThrow.throwError), Monad0: () => monadProxy1};
};
  ...
  return {
    catchError: v => v1 => {
      ...
      if (v.tag === "M") {
        return $$$Proxy(
          "M",
          dictMonadError.catchError(Bind1.bind(v._1)(p$p => Applicative0.pure(proxyMonadError(dictMonadError).catchError(p$p)(v1))))(Control$dSemigroupoid.composeImpl(pure)(v1))
        );
      }
      $runtime.fail();
    },
