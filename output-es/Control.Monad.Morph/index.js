...
const foldFree = /* #__PURE__ */ Control$dMonad$dFree.foldFree(Control$dMonad$dFree.monadRecFree);
const identity = x => x;
const mfunctorYoneda = {hoist: dictMonad => Data$dYoneda.hoistYoneda};
const mmonadYoneda = {
  embed: dictMonad => f => Control$dSemigroupoid.composeImpl(f)(Data$dYoneda.lowerYoneda),
  MFunctor0: () => mfunctorYoneda,
  MonadTrans1: () => Data$dYoneda.monadTransYoneda
};
const mfunctorWriterT = {hoist: dictMonad => nat => m => nat(m)};
const mmonadWriterT = dictMonoid => {
  const Semigroup0 = dictMonoid.Semigroup0();
  const monadTransWriterT = {
    lift: dictMonad => {
  ...
  return {
    embed: dictMonad => {
      ...
      return f => m => Bind1.bind(f(m))(v => Applicative0.pure(Data$dTuple.$Tuple(v._1._1, Semigroup0.append(v._1._2)(v._2))));
    },
...
const mmonadMaybeT = {
  embed: dictMonad => {
    const Bind1 = dictMonad.Bind1();
    const Applicative0 = dictMonad.Applicative0();
    return f => m => Bind1.bind(f(m))(x => Applicative0.pure((() => {
...
const mmonadFree = {embed: dictMonad => foldFree, MFunctor0: () => mfunctorFree, MonadTrans1: () => Control$dMonad$dFree.monadTransFree};
...
const generalize = dictMonad => Control$dSemigroupoid.composeImpl(dictMonad.Applicative0().pure)(Unsafe$dCoerce.unsafeCoerce);
const embed = dict => dict.embed;
const flipEmbed = dictMMonad => dictMonad => t => f => dictMMonad.embed(dictMonad)(f)(t);
const squash = dictMonad => dictMMonad => dictMMonad.embed(dictMonad)(identity);
const composeKleisliRight = dictMMonad => dictMonad => f => g => m => dictMMonad.embed(dictMonad)(g)(f(m));
const composeKleisliLeft = dictMMonad => dictMonad => g => f => m => dictMMonad.embed(dictMonad)(g)(f(m));
export {
  composeKleisliLeft,
  composeKleisliRight,
  embed,
