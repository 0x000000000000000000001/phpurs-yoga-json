  ...
  return dictBoundedEnum => {
    ...
    return {
      succ: v => {
        ...
        const $6 = Control$dSemigroupoid.composeImpl(Data$dMaybe.Just)(Data$dTuple.Tuple(v._1));
...
const enumFromThenTo = dictUnfoldable => dictFunctor => dictBoundedEnum => {
  const toEnum1 = dictBoundedEnum.toEnum;
  return a => b => c => {
    const a$p = dictBoundedEnum.fromEnum(a);
    return dictFunctor.map(Control$dSemigroupoid.composeImpl(fromJust)(toEnum1))(dictUnfoldable.unfoldr((() => {
...
const enumEither = dictBoundedEnum => {
  ...
  return dictBoundedEnum1 => {
    ...
    return {
      succ: v => {
        if (v.tag === "Left") {
          const $0 = Control$dSemigroupoid.composeImpl(Data$dMaybe.Just)(Data$dEither.Left);
          const $1 = Enum1.succ(v._1);
          if ($1.tag === "Nothing") { return Data$dMaybe.$Maybe("Just", Data$dEither.$Either("Right", Bounded01.bottom)); }
          if ($1.tag === "Just") { return $0($1._1); }
          $runtime.fail();
        }
        if (v.tag === "Right") {
          const $0 = Control$dSemigroupoid.composeImpl(Data$dMaybe.Just)(Data$dEither.Right);
          const $1 = Enum11.succ(v._1);
      ...
      pred: v => {
        ...
        if (v.tag === "Right") {
          const $0 = Control$dSemigroupoid.composeImpl(Data$dMaybe.Just)(Data$dEither.Right);
          const $1 = Enum11.pred(v._1);
          if ($1.tag === "Nothing") { return Data$dMaybe.$Maybe("Just", Data$dEither.$Either("Left", Bounded0.top)); }
          if ($1.tag === "Just") { return $0($1._1); }
        }
        $runtime.fail();
      },
...
const downFrom = dictEnum => {
  const pred1 = dictEnum.pred;
  return dictUnfoldable => dictUnfoldable.unfoldr(Control$dSemigroupoid.composeImpl(v1 => {
    if (v1.tag === "Just") { return Data$dMaybe.$Maybe("Just", Data$dTuple.$Tuple(v1._1, v1._1)); }
    return Data$dMaybe.Nothing;
  })(pred1));
};
const upFrom = dictEnum => {
  const succ1 = dictEnum.succ;
  return dictUnfoldable => dictUnfoldable.unfoldr(Control$dSemigroupoid.composeImpl(v1 => {
    if (v1.tag === "Just") { return Data$dMaybe.$Maybe("Just", Data$dTuple.$Tuple(v1._1, v1._1)); }
    return Data$dMaybe.Nothing;
  })(succ1));
