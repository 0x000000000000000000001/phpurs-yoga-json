...
const genericToEnum = dictGeneric => dictGenericBoundedEnum => Control$dSemigroupoid.composeImpl(v1 => {
  if (v1.tag === "Just") { return Data$dMaybe.$Maybe("Just", dictGeneric.to(v1._1)); }
  return Data$dMaybe.Nothing;
})(dictGenericBoundedEnum["genericToEnum'"]);
const genericSucc$p = dict => dict["genericSucc'"];
const genericSucc = dictGeneric => {
  const from = dictGeneric.from;
  return dictGenericEnum => Control$dSemigroupoid.composeImpl(v1 => {
    if (v1.tag === "Just") { return Data$dMaybe.$Maybe("Just", dictGeneric.to(v1._1)); }
    return Data$dMaybe.Nothing;
  })(Control$dSemigroupoid.composeImpl(dictGenericEnum["genericSucc'"])(from));
};
const genericPred$p = dict => dict["genericPred'"];
const genericPred = dictGeneric => {
  const from = dictGeneric.from;
  return dictGenericEnum => Control$dSemigroupoid.composeImpl(v1 => {
    if (v1.tag === "Just") { return Data$dMaybe.$Maybe("Just", dictGeneric.to(v1._1)); }
    return Data$dMaybe.Nothing;
  })(Control$dSemigroupoid.composeImpl(dictGenericEnum["genericPred'"])(from));
};
const genericFromEnum$p = dict => dict["genericFromEnum'"];
const genericFromEnum = dictGeneric => {
  const from = dictGeneric.from;
  return dictGenericBoundedEnum => Control$dSemigroupoid.composeImpl(dictGenericBoundedEnum["genericFromEnum'"])(from);
};
