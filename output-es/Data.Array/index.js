...
const updateAtIndices = dictFoldable => us => xs => {
  const result = [...xs];
  Data$dFoldable.traverse_(Control$dMonad$dST$dInternal.applicativeST)(dictFoldable)(v => {
    const $0 = v._2;
    const $1 = v._1;
...
const modifyAtIndices = dictFoldable => is => f => xs => {
  const result = [...xs];
  Data$dFoldable.traverse_(Control$dMonad$dST$dInternal.applicativeST)(dictFoldable)(i => Data$dArray$dST.modify(i)(f)(result))(is)();
  return result;
};
  ...
  if (0 < indexedAndSorted.length) {
    return Data$dFunctor.arrayMap(Data$dTuple.snd)(sortWith(Data$dOrd.ordInt)(Data$dTuple.fst)((() => {
      ...
      for (const v1 of indexedAndSorted) {
        if (comp(Control$dSemigroupoid.composeImpl(Data$dTuple.snd)(Control$dSemigroupoid.composeImpl(fromJust)(last))(result))(v1._2) !== "EQ") { result.push(v1); }
      }
      return result;
    })()));
  }
...
const groupAllBy = cmp => Control$dSemigroupoid.composeImpl(groupBy(x => y => cmp(x)(y) === "EQ"))(sortBy(cmp));
...
const mapMaybe = f => concatMap(Control$dSemigroupoid.composeImpl(v2 => {
...
const filterA = dictApplicative => {
  ...
  return p => Control$dSemigroupoid.composeImpl(Functor0.map(mapMaybe(v => {
    if (v._2) { return Data$dMaybe.$Maybe("Just", v._1); }
    return Data$dMaybe.Nothing;
  })))(Data$dTraversable.traversableArray.traverse(dictApplicative)(x => Functor0.map(Data$dTuple.Tuple(x))(p(x))));
};
...
const nubByEq = eq => xs => {
  const arr = [];
  for (const x of xs) {
    if (Control$dSemigroupoid.composeImpl(Data$dHeytingAlgebra.boolNot)(any(v => eq(v)(x)))(arr)) { arr.push(x); }
  }
  return arr;
};
