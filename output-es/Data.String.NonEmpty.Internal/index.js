...
const liftS = f => v => f(v);
const startsWith = /* #__PURE__ */ Control$dSemigroupoid.composeImpl(liftS)(Data$dString$dCodeUnits.startsWith);
const joinWith1 = dictFoldable1 => {
  const Foldable0 = dictFoldable1.Foldable0();
  return v => Control$dSemigroupoid.composeImpl(NonEmptyString)(xs => Foldable0.foldl(v$1 => v1 => {
    if (v$1.init) { return {init: false, acc: v1}; }
    return {init: false, acc: v$1.acc + v + v1};
  })({init: true, acc: ""})(xs).acc);
};
const joinWith = dictFoldable => splice => Control$dSemigroupoid.composeImpl(xs => dictFoldable.foldl(v => v1 => {
  if (v.init) { return {init: false, acc: v1}; }
  return {init: false, acc: v.acc + splice + v1};
})({init: true, acc: ""})(xs).acc)(Unsafe$dCoerce.unsafeCoerce);
const join1With = dictFoldable1 => {
  const Foldable0 = dictFoldable1.Foldable0();
  return splice => Control$dSemigroupoid.composeImpl(NonEmptyString)(joinWith(Foldable0)(splice));
};
const fromString = v => {
...
const unsafeFromString = () => Control$dSemigroupoid.composeImpl(fromJust)(fromString);
const eqNonEmptyString = Data$dEq.eqString;
const eqNonEmptyReplacement = Data$dEq.eqString;
const endsWith = /* #__PURE__ */ Control$dSemigroupoid.composeImpl(liftS)(Data$dString$dCodeUnits.endsWith);
const contains = /* #__PURE__ */ Control$dSemigroupoid.composeImpl(liftS)(Data$dString$dCodeUnits.contains);
