...
const listens = dictMonadWriter => {
  ...
  return f => m => Bind1.bind(dictMonadWriter.listen(m))(v => Monad1.Applicative0().pure(Data$dTuple.$Tuple(v._1, f(v._2))));
};
const censor = dictMonadWriter => {
  const Monad1 = dictMonadWriter.MonadTell1().Monad1();
  const Bind1 = Monad1.Bind1();
  return f => m => dictMonadWriter.pass(Bind1.bind(m)(a => Monad1.Applicative0().pure(Data$dTuple.$Tuple(a, f))));
};
