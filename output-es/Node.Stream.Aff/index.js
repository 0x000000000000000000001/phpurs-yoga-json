...
const write = dictMonadAff => {
  const liftAff = dictMonadAff.liftAff;
  return w => bs => Control$dSemigroupoid.composeImpl(liftAff)(Effect$dAff.makeAff)(complete => () => {
    let removeDrain = () => {};
    ...
    oneWrite(0)();
    return Effect$dAff.effectCanceler(() => {
      const $0 = removeDrain;
      return $0();
    });
  });
...
const toStringUTF8 = dictMonadEffect => bs => dictMonadEffect.liftEffect(() => {
  ...
  const a$p = Node$dBuffer.unsafeFreeze($0)();
  return Node$dBuffer$dImmutable.toString(Node$dEncoding.UTF8)(a$p);
});
const readableToBuffers = dictMonadAff => r => dictMonadAff.liftAff(Effect$dAff._makeAff(
  Effect$dAff.isLeft,
...
const readableToString = dictMonadAff => {
  ...
  return r => enc => Bind1.bind(readableToBuffers(dictMonadAff)(r))(bufs => MonadEffect0.liftEffect(() => {
    const $0 = Node$dBuffer.concat(bufs)();
    const a$p = Node$dBuffer.unsafeFreeze($0)();
    return Node$dBuffer$dImmutable.toString(enc)(a$p);
  }));
};
const readableToStringUtf8 = dictMonadAff => r => readableToString(dictMonadAff)(r)(Node$dEncoding.UTF8);
const readSome = dictMonadAff => {
  const liftAff = dictMonadAff.liftAff;
  return r => Control$dSemigroupoid.composeImpl(liftAff)(Effect$dAff.makeAff)(complete => () => {
...
const readN = dictMonadAff => {
  const liftAff = dictMonadAff.liftAff;
  return r => n => Control$dSemigroupoid.composeImpl(liftAff)(Effect$dAff.makeAff)(complete => {
    if (n < 0) {
      ...
      if (isReadable) {
        ...
        const tryToRead = continuation => {
          const $0 = Effect.untilE(() => {
            ...
            if (v.tag === "Just") {
              bufs.push(v._1);
              const a$p = Node$dBuffer.unsafeFreeze(v._1)();
              const $0 = redRef;
              const red$p = redRef = $0 + Node$dBuffer$dImmutable.size(a$p) | 0;
...
const fromStringUTF8 = dictMonadEffect => s => dictMonadEffect.liftEffect((() => {
  const $0 = Node$dBuffer.unsafeThaw(Node$dBuffer$dImmutable.fromString(s)(Node$dEncoding.UTF8));
  return () => {
    const a$p = $0();
    return [a$p];
  };
})());
const end = dictMonadAff => {
  const liftAff = dictMonadAff.liftAff;
  return w => Control$dSemigroupoid.composeImpl(liftAff)(Effect$dAff.makeAff)(complete => () => {
