...
const next = dictMonad => {
  ...
  const go = p => {
    ...
    if (p.tag === "Respond") { return Applicative0.pure(Data$dEither.$Either("Right", Data$dTuple.$Tuple(p._1, p._2()))); }
    if (p.tag === "M") { return Bind1.bind(p._1)(go); }
    if (p.tag === "Pure") { return Applicative0.pure(Data$dEither.$Either("Left", p._1)); }
    $runtime.fail();
  };
  return go;
};
...
const each = dictMonad => {
  ...
  return dictFoldable => xs => dictFoldable.foldr(a => p => applyProxy.apply(applyProxy.Functor0().map(v => x => x)(Pipes$dInternal.$$$Proxy("Respond", a, Pipes$dInternal.Pure)))(p))(Pipes$dInternal.$$$Proxy(
    "Pure",
    undefined
  ))(xs);
};
const discard = dictMonad => {
  const Applicative0 = dictMonad.Applicative0();
