...
const utimes = file => atime => mtime => {
  const toEpochMilliseconds = Control$dSemigroupoid.composeImpl(Data$dDateTime$dInstant.unInstant)(Data$dDateTime$dInstant.fromDateTime);
  const $0 = $runtime.intDiv(Data$dInt.round(toEpochMilliseconds(atime)), 1000);
  const $1 = $runtime.intDiv(Data$dInt.round(toEpochMilliseconds(mtime)), 1000);
...
const fdNext = fd => buff => {
  const $0 = Node$dBuffer.unsafeFreeze(buff);
  return () => {
    const a$p = $0();
    return fdRead(fd)(buff)(0)(Node$dBuffer$dImmutable.size(a$p))(Data$dMaybe.Nothing)();
  };
};
...
const fdAppend = fd => buff => {
  ...
  return () => {
    const a$p = $0();
    return fdWrite(fd)(buff)(0)(Node$dBuffer$dImmutable.size(a$p))(Data$dMaybe.Nothing)();
  };
};
...
const access$p = path => mode => {
  const $0 = Effect$dException.catchException(Control$dSemigroupoid.composeImpl(Effect.pureE)(Data$dEither.Left))(() => {
