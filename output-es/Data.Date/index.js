...
const lastDayOfMonth = y => m => {
  const unsafeDay = Control$dSemigroupoid.composeImpl(v => {
    if (v.tag === "Just") { return v._1; }
    $runtime.fail();
  })(Data$dDate$dComponent.boundedEnumDay.toEnum);
  if (m === "January") { return unsafeDay(31); }
  if (m === "February") {
    if (isLeapYear(y)) { return unsafeDay(29); }
    return unsafeDay(28);
  }
  if (m === "March") { return unsafeDay(31); }
  if (m === "April") { return unsafeDay(30); }
  if (m === "May") { return unsafeDay(31); }
  if (m === "June") { return unsafeDay(30); }
  if (m === "July") { return unsafeDay(31); }
  if (m === "August") { return unsafeDay(31); }
  if (m === "September") { return unsafeDay(30); }
  if (m === "October") { return unsafeDay(31); }
  if (m === "November") { return unsafeDay(30); }
  if (m === "December") { return unsafeDay(31); }
  $runtime.fail();
};
const eqDate = {
  eq: x => y => x._1 === y._1 && (() => {
...
const enumDate = {
  succ: v => {
    ...
    const v1 = Data$dDate$dComponent.enumDay.succ(v._3);
    if (ordMaybe.compare(v1)(Data$dMaybe.$Maybe("Just", lastDayOfMonth(v._1)(v._2))) === "GT") {
      const $0 = (() => {
        ...
        ? Data$dDate$dComponent.enumYear.succ(v._1)
  ...
  pred: v => {
    ...
    const pd = Data$dDate$dComponent.enumDay.pred(v._3);
    const m$p = (() => {
      if (
        (() => {
    ...
    const $0 = (() => {
      ...
      ? Data$dDate$dComponent.enumYear.pred(v._1)
      : Data$dMaybe.$Maybe("Just", v._1);
    if ($0.tag === "Just") {
      const $1 = $$Date($0._1)(m$p);
      if (
        (() => {
          if (pd.tag === "Nothing") { return true; }
          if (pd.tag === "Just") { return false; }
        ...
        return Data$dMaybe.$Maybe("Just", $1(l));
      }
      if (pd.tag === "Just") { return Data$dMaybe.$Maybe("Just", $1(pd._1)); }
    }
    return Data$dMaybe.Nothing;
  },
  Ord0: () => ordDate
