...
const traverse = () => () => () => () => dictApplicative => {
  ...
  return r => traverseSome()()()()(Functor0)(r)(Control$dSemigroupoid.composeImpl(pure)(Unsafe$dCoerce.unsafeCoerce));
};
...
const showVariantCons = dictVariantShows => dictShow => {
  ...
  return {variantShows: v => Data$dList$dTypes.$List("Cons", show, dictVariantShows.variantShows(Type$dProxy.Proxy))};
};
...
const prj = () => dictIsSymbol => dictAlternative => {
  ...
  return p => onImpl(dictIsSymbol.reflectSymbol)(p)(pure)((() => {
    const $0 = Plus1.empty;
    return v => $0;
  })());
};
const inj = () => dictIsSymbol => p => value => ({type: dictIsSymbol.reflectSymbol(p), value});
const overOne = dictIsSymbol => () => () => p => f => onImpl(dictIsSymbol.reflectSymbol)(p)(Control$dSemigroupoid.composeImpl(value => (
  {type: dictIsSymbol.reflectSymbol(p), value}
))(f));
const revariant = v => v(dictIsSymbol => () => p => value => ({type: dictIsSymbol.reflectSymbol(p), value}));
const traverseOne = dictIsSymbol => () => () => dictFunctor => p => f => onImpl(dictIsSymbol.reflectSymbol)(p)(Control$dSemigroupoid.composeImpl(dictFunctor.map(value => (
  {type: dictIsSymbol.reflectSymbol(p), value}
)))(f));
...
const eqVariantCons = dictVariantEqs => dictEq => {
  const eq = dictEq.eq;
  return {variantEqs: v => Data$dList$dTypes.$List("Cons", eq, dictVariantEqs.variantEqs(Type$dProxy.Proxy))};
};
...
const enumVariant = () => dictVariantTags => dictVariantEqs => {
  ...
  return dictVariantOrds => {
    ...
    return dictVariantBoundedEnums => {
      const VariantBounded0 = dictVariantBoundedEnums.VariantBounded0();
      return {
        pred: a => Data$dVariant$dInternal.lookupPred(a)(dictVariantTags.variantTags(Type$dProxy.Proxy))(VariantBounded0.variantBounded(Type$dProxy.Proxy))(dictVariantBoundedEnums.variantBoundedEnums(Type$dProxy.Proxy)),
        succ: a => Data$dVariant$dInternal.lookupSucc(a)(dictVariantTags.variantTags(Type$dProxy.Proxy))(VariantBounded0.variantBounded(Type$dProxy.Proxy))(dictVariantBoundedEnums.variantBoundedEnums(Type$dProxy.Proxy)),
        Ord0: () => ordVariant4
      };
    };
  };
