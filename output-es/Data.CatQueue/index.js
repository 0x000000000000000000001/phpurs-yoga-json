...
const showCatQueue = dictShow => {
  ...
  return {show: v => "(CatQueue " + showList.show(v._1) + " " + showList.show(v._2) + ")"};
};
...
const foldableCatQueue = {
  foldMap: dictMonoid => {
    ...
    return f => foldableCatQueue.foldl(acc => x => Semigroup0.append(acc)(f(x)))(mempty);
  },
...
const fromFoldable = dictFoldable => f => dictFoldable.foldMap(monoidCatQueue)(singleton)(f);
const traversableCatQueue = {
  traverse: dictApplicative => {
    const Functor0 = dictApplicative.Apply0().Functor0();
    const Apply0 = dictApplicative.Apply0();
    return f => Control$dSemigroupoid.composeImpl(Functor0.map(foldableCatQueue.foldl(snoc)($CatQueue(Data$dList$dTypes.Nil, Data$dList$dTypes.Nil))))(foldableCatQueue.foldl(acc => Control$dSemigroupoid.composeImpl((() => {
      const Functor0$1 = Apply0.Functor0();
      return b => Apply0.apply(Functor0$1.map(snoc)(acc))(b);
    })())(f))(dictApplicative.pure($CatQueue(Data$dList$dTypes.Nil, Data$dList$dTypes.Nil))));
  },
