...
const traversableLazy = {
  traverse: dictApplicative => {
    ...
    return f => l => Functor0.map(Control$dSemigroupoid.composeImpl(defer)(Data$dFunction.const))(f(force(l)));
  },
  sequence: dictApplicative => {
    const Functor0 = dictApplicative.Apply0().Functor0();
    return l => Functor0.map(Control$dSemigroupoid.composeImpl(defer)(Data$dFunction.const))(force(l));
  },
...
const euclideanRingLazy = dictEuclideanRing => {
  ...
  return {
    degree: Control$dSemigroupoid.composeImpl(dictEuclideanRing.degree)(force),
