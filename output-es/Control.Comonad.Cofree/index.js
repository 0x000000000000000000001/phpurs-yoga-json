...
const hoistCofree = dictFunctor => nat => v => {
  const $0 = Control$dSemigroupoid.composeImpl(nat)(dictFunctor.map(hoistCofree(dictFunctor)(nat)));
...
const foldableCofree = dictFoldable => (
  {
    ...
    foldMap: dictMonoid => {
      ...
      return f => {
        const go = fa => Semigroup0.append(f(Data$dLazy.force(fa)._1))(dictFoldable.foldMap(dictMonoid)(go)(Data$dLazy.force(fa)._2));
        return go;
      };
    }
  }
...
const foldableWithIndexCofree = dictFoldable => {
  ...
  return {
    ...
    foldMapWithIndex: dictMonoid => {
      const Semigroup0 = dictMonoid.Semigroup0();
      return f => {
        const go = ix => fa => Semigroup0.append(f(ix)(Data$dLazy.force(fa)._1))(dictFoldable.foldMap(dictMonoid)(go(ix + 1 | 0))(Data$dLazy.force(fa)._2));
        return go(0);
      };
    },
...
const traversableCofree = dictTraversable => {
  ...
  return {
    ...
    traverse: dictApplicative => {
      ...
      return f => {
        const loop = ta => Apply0.apply(Functor0.map(mkCofree)(f(Data$dLazy.force(ta)._1)))(dictTraversable.traverse(dictApplicative)(loop)(Data$dLazy.force(ta)._2));
        return loop;
      };
    },
    Functor0: () => functorCofree1,
...
const traversableWithIndexCofree = dictTraversable => {
  ...
  return {
    traverseWithIndex: dictApplicative => {
      ...
      const Functor0 = dictApplicative.Apply0().Functor0();
      return f => {
        const loop = ix => ta => Apply0.apply(Functor0.map(mkCofree)(f(ix)(Data$dLazy.force(ta)._1)))(dictTraversable.traverse(dictApplicative)(loop(ix + 1 | 0))(Data$dLazy.force(ta)._2));
        return loop(0);
      };
    },
...
const deferCofree = /* #__PURE__ */ Control$dSemigroupoid.composeImpl(Cofree)(Data$dLazy.defer);
const semigroupCofree = dictApply => {
  const Functor0 = dictApply.Functor0();
  return dictSemigroup => (
    {
      append: x => y => deferCofree(v => Data$dTuple.$Tuple(
        dictSemigroup.append(Data$dLazy.force(x)._1)(Data$dLazy.force(y)._1),
        dictApply.apply(Functor0.map(semigroupCofree(dictApply)(dictSemigroup).append)(Data$dLazy.force(x)._2))(Data$dLazy.force(y)._2)
      ))
    }
  );
};
const monoidCofree = dictApplicative => {
  ...
  return dictMonoid => {
    ...
    return {
      mempty: deferCofree(v => Data$dTuple.$Tuple(dictMonoid.mempty, dictApplicative.pure(monoidCofree(dictApplicative)(dictMonoid).mempty))),
...
const explore = dictFunctor => dictFunctor1 => pair => m => w => {
  const v = Control$dSemigroupoid.composeImpl(Unsafe$dCoerce.unsafeCoerce)(Control$dMonad$dFree.runFreeM(dictFunctor)(monadRecStateT)(ff => monadStateStateT.state(cof => pair(dictFunctor.map(Data$dTuple.Tuple)(ff))(Data$dLazy.force(cof)._2)))(m))(w);
...
const exploreM = dictFunctor => dictFunctor1 => dictMonadRec => {
  ...
  const monadRecStateT1 = Control$dMonad$dState$dTrans.monadRecStateT(dictMonadRec);
  return pair => m => w => Functor0.map(v => v._1(Data$dLazy.force(v._2)._1))(Control$dMonad$dFree.runFreeM(dictFunctor)(monadRecStateT1)(ff => cof => pair(dictFunctor.map(Data$dTuple.Tuple)(ff))(Data$dLazy.force(cof)._2))(m)(w));
};
  ...
  return {
    bind: fa => f => {
      const loop = fa$p => {
        ...
        const $1 = Alt0.alt(Data$dLazy.force(fh)._2)(Functor0.map(loop)(Data$dLazy.force(fa$p)._2));
...
const applyCofree = dictAlternative => {
  ...
  return {
    apply: (() => {
      const Bind1 = bindCofree(dictAlternative);
      const Applicative0 = applicativeCofree(dictAlternative);
