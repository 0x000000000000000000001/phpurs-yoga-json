...
const fromCharCode = /* #__PURE__ */ Control$dSemigroupoid.composeImpl(Data$dString$dCodeUnits.singleton)(x => {
...
const uncons = v => {
  ...
  const h = unsafeCodePointAt0(v);
  return Data$dMaybe.$Maybe("Just", {head: h, tail: Data$dString$dCodeUnits.drop(Data$dString$dCodeUnits.length(singleton(h)))(v)});
};
...
const length = /* #__PURE__ */ Control$dSemigroupoid.composeImpl(Data$dArray.length)(toCodePointArray);
const indexOf = p => s => {
  const $0 = Data$dString$dCodeUnits.indexOf(p)(s);
  if ($0.tag === "Just") { return Data$dMaybe.$Maybe("Just", length(Data$dString$dCodeUnits.take($0._1)(s))); }
  return Data$dMaybe.Nothing;
};
...
const indexOf$p = p => i => s => {
  ...
  if ($0.tag === "Just") { return Data$dMaybe.$Maybe("Just", i + length(Data$dString$dCodeUnits.take($0._1)(s$p)) | 0); }
  return Data$dMaybe.Nothing;
};
...
const codePointFromChar = /* #__PURE__ */ Control$dSemigroupoid.composeImpl(CodePoint)(Data$dEnum.toCharCode);
