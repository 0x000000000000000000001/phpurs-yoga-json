...
const millisToTime = v => {
  ...
  const $0 = Data$dInt.floor(hours);
  if ($0 >= 0 && $0 <= 23) {
    const $1 = Data$dInt.floor(minutes);
    if ($1 >= 0 && $1 <= 59) {
      const $2 = Data$dInt.floor(seconds);
      if ($2 >= 0 && $2 <= 59) {
        const $3 = Data$dInt.floor(v - (hours * 3600000.0 + minutes * 60000.0 + seconds * 1000.0));
        if ($3 >= 0 && $3 <= 999) { return $Time($0, $1, $2, $3); }
        $runtime.fail();
      }
      const $3 = Data$dInt.floor(v - (hours * 3600000.0 + minutes * 60000.0 + seconds * 1000.0));
      $runtime.fail();
...
const diff = dictDuration => t1 => t2 => dictDuration.toDuration(timeToMillis(t1) + Control$dSemigroupoid.composeImpl(Data$dTime$dDuration.identity)(Control$dSemigroupoid.composeImpl(Data$dTime$dDuration.negate)(Data$dTime$dDuration.identity))(timeToMillis(t2)));
...
const adjust = dictDuration => d => t => {
  ...
  const msAdjusted = timeToMillis(t) + d$p + Control$dSemigroupoid.composeImpl(Data$dTime$dDuration.identity)(Control$dSemigroupoid.composeImpl(Data$dTime$dDuration.negate)(Data$dTime$dDuration.identity))(wholeDays * 86400000.0);
  const wrap = (() => {
