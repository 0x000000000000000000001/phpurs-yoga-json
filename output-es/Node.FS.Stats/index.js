...
const statusChangedTime = s => {
  const v = Data$dJSDate.toDateTime(statusChangedTimeImpl(s));
  if (v.tag === "Just") { return v._1; }
  if (v.tag === "Nothing") { return Partial._crashWith("Impossible: `statusChangedTime` returned invalid DateTime value."); }
  $runtime.fail();
};
...
const modifiedTime = s => {
  const v = Data$dJSDate.toDateTime(modifiedTimeImpl(s));
  if (v.tag === "Just") { return v._1; }
  if (v.tag === "Nothing") { return Partial._crashWith("Impossible: `modifiedTime` returned invalid DateTime value."); }
  $runtime.fail();
};
...
const birthTime = s => {
  const v = Data$dJSDate.toDateTime(birthTimeImpl(s));
  if (v.tag === "Just") { return v._1; }
  if (v.tag === "Nothing") { return Partial._crashWith("Impossible: `birthTime` returned invalid DateTime value."); }
  $runtime.fail();
};
const accessedTimeMs = s => accessedTimeMsImpl(s);
const accessedTime = s => {
  const v = Data$dJSDate.toDateTime(accessedTimeImpl(s));
  if (v.tag === "Just") { return v._1; }
  if (v.tag === "Nothing") { return Partial._crashWith("Impossible: `accessedTime` returned invalid DateTime value."); }
  $runtime.fail();
};
export {
  accessedTime,
