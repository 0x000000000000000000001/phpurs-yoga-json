...
const readEither$p = r => size => () => {
  ...
  if (v.tag === "Just") {
    return readChunkImpl(
      $0 => Control$dSemigroupoid.composeImpl(Effect.pureE)(Control$dSemigroupoid.composeImpl(Data$dMaybe.Just)(Data$dEither.Right))($0)(),
      $0 => Control$dSemigroupoid.composeImpl(Effect.pureE)(Control$dSemigroupoid.composeImpl(Data$dMaybe.Just)(Data$dEither.Left))($0)(),
      v._1
    );
  }
  $runtime.fail();
...
const read$p = r => size => () => {
  ...
  const v = Data$dNullable.nullable(chunk, Data$dMaybe.Nothing, Data$dMaybe.Just);
  if (v.tag === "Nothing") { return Data$dMaybe.Nothing; }
  if (v.tag === "Just") { return readChunkImpl(buf => Data$dMaybe.$Maybe("Just", buf), v1 => Effect$dException.throw("Stream encoding should not be set")(), v._1); }
  $runtime.fail();
};
const readString$p = r => size => enc => {
  ...
  return () => {
    ...
    if (mbBuf.tag === "Just") {
      const a$p = Node$dBuffer.unsafeFreeze(mbBuf._1)();
      return Data$dMaybe.$Maybe("Just", Node$dBuffer$dImmutable.toString(enc)(a$p));
    }
    $runtime.fail();
  };
};
...
const dataHStr = /* #__PURE__ */ Node$dEventEmitter.$EventHandle(
  "data",
  cb => chunk => readChunkImpl(v => Effect$dException.throw("Got a Buffer, not String. Stream encoding must be set to get a String.")(), $0 => cb($0)(), chunk)
);
const dataHEither = /* #__PURE__ */ Node$dEventEmitter.$EventHandle(
  "data",
  cb => chunk => readChunkImpl($0 => Control$dSemigroupoid.composeImpl(cb)(Data$dEither.Left)($0)(), $0 => Control$dSemigroupoid.composeImpl(cb)(Data$dEither.Right)($0)(), chunk)
);
const dataH = /* #__PURE__ */ Node$dEventEmitter.$EventHandle(
  "data",
  cb => chunk => readChunkImpl($0 => cb($0)(), v => Effect$dException.throw("Got a String, not a Buffer. Stream encoding should not be set")(), chunk)
);
