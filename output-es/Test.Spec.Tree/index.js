...
const showRecord1 = {
  show: record => "{ example: " + Data$dShow.showStringImpl(record.example) + "," + (record.isFocused ? " isFocused: true," : " isFocused: false,") + (() => {
...
const showItem = {show: v => "Item (" + showRecord1.show({isFocused: v.isFocused, isParallelizable: v.isParallelizable, example: "Function"}) + ")"};
const ordPathItem = /* #__PURE__ */ (() => {
  const eqRec1 = {
  ...
  return {
    compare: ra => rb => {
      ...
      if (left$1 !== "EQ") { return left$1; }
      return Data$dOrdering.EQ;
    },
    Eq0: () => eqRec1
  };
})();
const newtypePathItem_ = {Coercible0: () => {}};
const newtypeItem_ = {Coercible0: () => {}};
const functorTree = {
  map: f => m => {
...
const foldableTree = {
  ...
  foldMap: dictMonoid => f => {
    ...
    return foldableTree.foldl(acc => x => Semigroup0.append(acc)(f(x)))(dictMonoid.mempty);
  }
};
...
const parentSuiteName = /* #__PURE__ */ Data$dArray.mapMaybe(/* #__PURE__ */ Control$dSemigroupoid.composeImpl(v => v.name)(Unsafe$dCoerce.unsafeCoerce));
const parentSuite = path => {
  const $0 = Data$dArray.unsnoc(path);
  if ($0.tag === "Just") {
    if ($0._1.last.name.tag === "Just") { return Data$dMaybe.$Maybe("Just", Data$dTuple.$Tuple($0._1.init, $0._1.last.name._1)); }
    if ($0._1.last.name.tag === "Nothing") { return Data$dMaybe.Nothing; }
    $runtime.fail();
  }
  if ($0.tag === "Nothing") { return Data$dMaybe.Nothing; }
  $runtime.fail();
};
const modifyAroundAction = action => v => ({...v, example: aroundAction => v.example(Control$dSemigroupoid.composeImpl(aroundAction)(action))});
...
const isAllParallelizable = v => {
  if (v.tag === "Node") {
    return Data$dFoldable.foldableArray.foldMap((() => {
      const semigroupConj1 = {append: v$1 => v1 => v$1 && v1};
      return {mempty: true, Semigroup0: () => semigroupConj1};
    })())(isAllParallelizable)(v._2);
  }
  if (v.tag === "Leaf") {
    const $0 = Control$dSemigroupoid.composeImpl(v1 => v1.isParallelizable)(Unsafe$dCoerce.unsafeCoerce);
    return v._2.tag === "Nothing" || (() => {
      const $1 = (() => {
        if (v._2.tag === "Just") { return $0(v._2._1); }
        if (v._2.tag === "Nothing") { return Data$dMaybe.Nothing; }
        $runtime.fail();
      })();
      if ($1.tag === "Nothing") { return false; }
      return $1.tag === "Just" && $1._1;
    })();
  }
  $runtime.fail();
};
...
const countTests = g => {
  const go = v => {
    if (v.tag === "Node") { return Data$dFoldable.for_(applicativeStateT)(Data$dFoldable.foldableArray)(v._2)(go); }
    if (v.tag === "Leaf") { return monadStateStateT.state(s => Data$dTuple.$Tuple(undefined, s + 1 | 0)); }
    $runtime.fail();
  };
  return Data$dTraversable.traversableArray.traverse(applicativeStateT)(go)(g)(0)._2;
};
...
const annotatedWithPaths = /* #__PURE__ */ (() => {
  const go = path => index => v => {
    if (v.tag === "Node") {
      return $Tree(
        "Node",
        (() => {
          if (v._1.tag === "Left") { return Data$dEither.$Either("Left", Data$dTuple.$Tuple(path, v._1._1)); }
          if (v._1.tag === "Right") { return Data$dEither.$Either("Right", v._1._1); }
          $runtime.fail();
        })(),
    ...
    if (v.tag === "Leaf") { return $Tree("Leaf", Data$dTuple.$Tuple(path, v._1), v._2); }
    $runtime.fail();
  };
