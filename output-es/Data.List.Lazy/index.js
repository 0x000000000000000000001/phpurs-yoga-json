...
const uncons = xs => {
  const v = Data$dList$dLazy$dTypes.step(xs);
...
const takeWhile = p => Control$dSemigroupoid.composeImpl(Data$dList$dLazy$dTypes.List)(Control$dSemigroupoid.composeImpl(Data$dLazy.functorLazy.map(v => {
...
const take = n => {
  ...
  return Control$dSemigroupoid.composeImpl(Data$dList$dLazy$dTypes.List)(Control$dSemigroupoid.composeImpl(Data$dLazy.functorLazy.map(v1 => {
    if (v1.tag === "Nil") { return Data$dList$dLazy$dTypes.Nil; }
...
const stripPrefix = dictEq => v => s => Control$dMonad$dRec$dClass.monadRecMaybe.tailRecM(o => {
  const v1 = Data$dList$dLazy$dTypes.step(o.a);
  if (v1.tag === "Nil") { return Data$dMaybe.$Maybe("Just", Control$dMonad$dRec$dClass.$Step("Done", o.b)); }
  if (v1.tag === "Cons") {
    const v2 = Data$dList$dLazy$dTypes.step(o.b);
...
const reverse = xs => Data$dLazy.defer(Control$dSemigroupoid.composeImpl(Data$dList$dLazy$dTypes.step)(v => Data$dList$dLazy$dTypes.foldableList.foldl(b => a => Data$dLazy.defer(v$1 => Data$dList$dLazy$dTypes.$Step(
  "Cons",
  a,
  b
)))(Data$dList$dLazy$dTypes.nil)(xs)));
...
const repeat = x => {
  const go$lazy = $runtime.binding(() => Data$dLazy.defer(Control$dSemigroupoid.composeImpl(Data$dList$dLazy$dTypes.step)(v => Data$dLazy.defer(v$1 => Data$dList$dLazy$dTypes.$Step(
    "Cons",
    x,
    go$lazy()
  )))));
...
const $$null = /* #__PURE__ */ Control$dSemigroupoid.composeImpl(Data$dMaybe.isNothing)(uncons);
const nubBy = p => {
  const goStep = v => v1 => {
    if (v1.tag === "Nil") { return Data$dList$dLazy$dTypes.Nil; }
    if (v1.tag === "Cons") {
      const v2 = Data$dList$dInternal.insertAndLookupBy(p)(v1._1)(v);
      if (v2.found) { return Data$dList$dLazy$dTypes.step(go(v2.result)(v1._2)); }
...
const mapMaybe = f => {
  const go = go$a0$copy => {
    ...
    while (go$c) {
      ...
      if (v.tag === "Cons") {
        const v1 = f(v._1);
        if (v1.tag === "Nothing") {
          go$a0 = Data$dList$dLazy$dTypes.step(v._2);
          continue;
        }
        if (v1.tag === "Just") {
          go$c = false;
  ...
  return Control$dSemigroupoid.composeImpl(Data$dList$dLazy$dTypes.List)(Control$dSemigroupoid.composeImpl(Data$dLazy.functorLazy.map(go))(Unsafe$dCoerce.unsafeCoerce));
};
...
const last = /* #__PURE__ */ (() => {
  ...
  return Control$dSemigroupoid.composeImpl(go)(Data$dList$dLazy$dTypes.step);
})();
const iterate = f => x => {
  const go$lazy = $runtime.binding(() => Data$dLazy.defer(Control$dSemigroupoid.composeImpl(Data$dList$dLazy$dTypes.step)(v => {
    const $0 = Data$dList$dLazy$dTypes.functorList.map(f)(go$lazy());
    return Data$dLazy.defer(v$1 => Data$dList$dLazy$dTypes.$Step("Cons", x, $0));
  })));
  const go = go$lazy();
  return go;
};
...
const init = /* #__PURE__ */ (() => {
  const go = v => {
    if (v.tag === "Cons") {
      if ($$null(v._2)) { return Data$dMaybe.$Maybe("Just", Data$dList$dLazy$dTypes.nil); }
      const $0 = go(Data$dList$dLazy$dTypes.step(v._2));
      if ($0.tag === "Just") {
...
const index = xs => {
  ...
  return go(Data$dList$dLazy$dTypes.step(xs));
};
const head = xs => {
  const $0 = uncons(xs);
...
const groupBy = eq => Control$dSemigroupoid.composeImpl(Data$dList$dLazy$dTypes.List)(Control$dSemigroupoid.composeImpl(Data$dLazy.functorLazy.map(v => {
...
const fromStep = /* #__PURE__ */ (() => Control$dSemigroupoid.composeImpl(Data$dList$dLazy$dTypes.List)(Data$dLazy.applicativeLazy.pure))();
const insertBy = cmp => x => xs => Data$dLazy.defer(v => {
  ...
  if ($0.tag === "Cons") {
    ...
    return Data$dList$dLazy$dTypes.$Step("Cons", x, fromStep($0));
  }
  $runtime.fail();
});
...
const nubByEq = eq => Control$dSemigroupoid.composeImpl(Data$dList$dLazy$dTypes.List)(Control$dSemigroupoid.composeImpl(Data$dLazy.functorLazy.map(v => {
  if (v.tag === "Nil") { return Data$dList$dLazy$dTypes.Nil; }
  if (v.tag === "Cons") {
    const $0 = v._1;
...
const drop = n => {
  const go = go$a0$copy => go$a1$copy => {
    ...
    while (go$c) {
      ...
      if (v1.tag === "Cons") {
        go$a0 = v - 1 | 0;
        go$a1 = Data$dList$dLazy$dTypes.step(v1._2);
        continue;
      }
      $runtime.fail();
    }
  ...
  return Control$dSemigroupoid.composeImpl(Data$dList$dLazy$dTypes.List)(Control$dSemigroupoid.composeImpl(Data$dLazy.functorLazy.map(go(n)))(Unsafe$dCoerce.unsafeCoerce));
};
...
const deleteBy = eq => x => xs => Data$dLazy.defer(v => {
  ...
  if ($0.tag === "Nil") { return Data$dList$dLazy$dTypes.Nil; }
  if ($0.tag === "Cons") {
    if (eq(x)($0._1)) { return Data$dList$dLazy$dTypes.step($0._2); }
...
const deleteAt = n => xs => Data$dLazy.defer(v => {
  ...
  if ($0.tag === "Nil") { return Data$dList$dLazy$dTypes.Nil; }
  if ($0.tag === "Cons") {
    if (n === 0) { return Data$dList$dLazy$dTypes.step($0._2); }
...
const cycle = xs => {
  const go$lazy = $runtime.binding(() => Data$dLazy.defer(Control$dSemigroupoid.composeImpl(Data$dList$dLazy$dTypes.step)(v => Data$dList$dLazy$dTypes.semigroupList.append(xs)(go$lazy()))));
  const go = go$lazy();
  return go;
};
...
const alterAt = n => f => xs => Data$dLazy.defer(v => {
  ...
  if ($0.tag === "Cons") {
    if (n === 0) {
      const v2 = f($0._1);
      if (v2.tag === "Nothing") { return Data$dList$dLazy$dTypes.step($0._2); }
...
const modifyAt = n => f => alterAt(n)(Control$dSemigroupoid.composeImpl(Data$dMaybe.Just)(f));
export {
  Pattern,
  alterAt,
  catMaybes,
