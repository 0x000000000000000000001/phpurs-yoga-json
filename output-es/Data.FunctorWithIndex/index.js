  ...
  return dictFunctorWithIndex1 => {
    ...
    return {
      mapWithIndex: f => v => Data$dTuple.$Tuple(
        dictFunctorWithIndex.mapWithIndex(Control$dSemigroupoid.composeImpl(f)(Data$dEither.Left))(v._1),
        dictFunctorWithIndex1.mapWithIndex(Control$dSemigroupoid.composeImpl(f)(Data$dEither.Right))(v._2)
      ),
...
const functorWithIndexCoproduct = dictFunctorWithIndex => {
  ...
  return dictFunctorWithIndex1 => {
    ...
    return {
      mapWithIndex: f => v => {
        const $2 = dictFunctorWithIndex.mapWithIndex(Control$dSemigroupoid.composeImpl(f)(Data$dEither.Left));
        const $3 = dictFunctorWithIndex1.mapWithIndex(Control$dSemigroupoid.composeImpl(f)(Data$dEither.Right));
        if (v.tag === "Left") { return Data$dEither.$Either("Left", $2(v._1)); }
        if (v.tag === "Right") { return Data$dEither.$Either("Right", $3(v._1)); }
        $runtime.fail();
      },
...
const functorWithIndexCompose = dictFunctorWithIndex => {
  const $0 = dictFunctorWithIndex.Functor0();
  return dictFunctorWithIndex1 => {
    const mapWithIndex1 = dictFunctorWithIndex1.mapWithIndex;
    const $1 = dictFunctorWithIndex1.Functor0();
    const functorCompose1 = {map: f => v => $0.map($1.map(f))(v)};
    return {
      mapWithIndex: f => v => dictFunctorWithIndex.mapWithIndex(Control$dSemigroupoid.composeImpl(mapWithIndex1)(a => b => f(Data$dTuple.$Tuple(a, b))))(v),
